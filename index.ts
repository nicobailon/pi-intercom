...
export default function piIntercomExtension(pi: ExtensionAPI) {
  ...
  function waitForReply(from: string, replyTo: string, signal?: AbortSignal): Promise<Message> {
    if (replyWaiters.has(replyTo)) {
      throw new Error(`Already waiting for reply ${replyTo}`);
    }
    if (signal?.aborted) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanup = () => {
        ...
        if (replyWaiters.get(replyTo) === waiter) {
      ...
      replyWaiters.set(replyTo, waiter);
    });
  }
  ...
  function handleIncomingMessage(ctx: ExtensionContext, from: SessionInfo, message: Message): void {
    ...
    if (message.replyTo) {
      const replyWaiter = replyWaiters.get(message.replyTo);
      if (replyWaiter) {
